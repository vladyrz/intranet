...
use App\Filament\Resources\AdminReminderPlusResource\Pages;
...
class AdminReminderPlusResource extends Resource
{
    protected static ?string $model = AdminReminderPlus::class;
                ...
                Forms\Components\Section::make('Status & History')
                    ->schema([
                        Forms\Components\Placeholder::make('next_run_at')
                            ->content(fn(?AdminReminderPlus $record) => $record?->next_run_at?->format('Y-m-d H:i:s') ?? 'Pending calculation'),

                        Forms\Components\Placeholder::make('last_sent_at')
                            ->content(fn(?AdminReminderPlus $record) => $record?->last_sent_at?->format('Y-m-d H:i:s') ?? 'Never'),

                        Forms\Components\Placeholder::make('failure_count')
                            ->content(fn(?AdminReminderPlus $record) => $record?->failure_count ?? 0),

                        Forms\Components\Placeholder::make('last_error_message')
                            ->content(fn(?AdminReminderPlus $record) => $record?->last_error_message ?? 'None')
                            ->visible(fn(?AdminReminderPlus $record) => !empty($record?->last_error_message)),
                    ])->columns(2)
                    ->collapsible()
                    ->collapsed(),
            ]);
    ...
    {
        return [
            'index' => Pages\ListAdminReminderPluses::route('/'),
            'create' => Pages\CreateAdminReminderPlus::route('/create'),
            'edit' => Pages\EditAdminReminderPlus::route('/{record}/edit'),
        ];
    }
}
