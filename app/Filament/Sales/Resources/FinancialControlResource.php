...
use App\Filament\Sales\Resources\FinancialControlResource\Pages;
        ...
        return $form
            ->schema([
                Section::make()
                    ->columns(12)
                    ->schema([
                        Select::make('country_id')->label('País')->relationship(name: 'country', titleAttribute: 'name')->searchable()->preload()->required()->columnSpan(3)->disabled(),
                        Select::make('user_id')->label('Propietario / Acreedor')->relationship(name: 'user', titleAttribute: 'name')->searchable()->preload()->required()->columnSpan(3)->disabled(),
                        Select::make('status')->label('Estado')->options([
                            'active' => 'Vigente',
                            'cancelled' => 'Cancelado',
                        ])->required()->columnSpan(3)->default('active')->disabled(),
                        Select::make('type')->label('Tipo')->options([
                            'loan' => 'Préstamo',
                            'invoice_advance' => 'Adelanto de facturas',
                            'investment' => 'Inversión',
                        ])->reactive()->required()->columnSpan(3)->disabled(),
                        Select::make('currency')->label('Moneda')->options([
                            'usd' => 'USD',
                            'crc' => 'CRC',
                        ])->required()->columnSpan(3)->default('usd')->disabled(),
                        TextInput::make('amount')->label('Monto')->numeric()->minValue(0)->default(0)->columnSpan(3)->disabled(),
                        TextInput::make('debtor')->label('Deudor')->columnSpan(3)->visible(fn(Get $get) => $get('type') === 'loan')->disabled(),
                        TextInput::make('invoice_number')->label('Número de factura')->columnSpan(3)->visible(fn(Get $get) => $get('type') === 'invoice_advance')->disabled(),
                        TextInput::make('responsible_person')->label('Responsable')->columnSpan(3)->visible(fn(Get $get) => $get('type') === 'invoice_advance')->disabled(),
                        TextInput::make('description')->label('Descripción')->columnSpan(4)->disabled(),
                        DatePicker::make('entry_date')->label('Fecha de ingreso')->columnSpan(4)->native(false)->disabled(),
                        Textarea::make('observations')->label('Observaciones')->columnSpan(4)->rows(3)->disabled(),
                    ]),
            ]);
    }
